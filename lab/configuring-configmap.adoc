...
This will make the configuration file available as the file _/etc/config/config.json_. Let's verify by going into the pod itself (remember how to do this by using _oc exec_? You can also use _oc exec_, but now you will learn another option: _oc rsh_):
